...
def pkgs_work(p):
    ...
    for f in p.functions:
        name = relaxName(p.name + "." + f.name)
        print('{} from {} to {}'.format(name, hex(f.offset), hex(f.end)))
        add_function(name, f.offset, f.end)
    print("Methods:")
    for m in p.methods:
        print('{} {} from {} to {}'.format(m.receiver, m.name, hex(m.offset), hex(m.end)))
        name = p.name
        if m.receiver != "":
            name += ".(" + m.receiver + ")"
        name += "." + m.name
        name = relaxName(name)
        add_function(name, m.offset, m.end)


def add_function(name, offset, end):
    ida_bytes.del_items(offset, 1, end - offset)
    idaapi.auto_make_proc(offset)
    ida_funcs.add_func(offset, end)
    rename(offset, name)
...
def relaxName(name):
    name = name.replace('.', '_').replace("<-", '_chan_left_').replace('*', '_ptr_')
    name = name.replace('-', '_').replace(';', '').replace('"', '').replace('\\', '')
    name = name.replace('(', '').replace(')', '').replace('/', '_').replace(' ', '_')
    name = name.replace(',', 'comma').replace('{', '').replace('}', '').replace('[', '').replace(']', '')
    return name
